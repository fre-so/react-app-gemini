...
import { type RefObject, useCallback, useEffect, useMemo, useRef, useState, type ComponentType } from 'react';
...
function buildMediaGroups(stepCount: number, getMediaKey: (stepIndex: number) => string | number) {
...
export default function StickySideScrollytelling({
  ...
  const handleProgressReady = useCallback((index: number, progress: MotionValue<number>) => {
...
function StickySideMediaGroup({
  ...
  return (
    ...
    >
      <MediaComponent stepIndex={stepIndex} isActive={isActive} scrollProgress={groupProgress} />
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      <div className={cn('w-full', stepClassName)}>
        <StepComponent stepIndex={stepIndex} isActive={isActive} scrollProgress={smoothProgress} />
      </div>
    </motion.article>
  );
}
