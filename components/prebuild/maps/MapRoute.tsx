...
import Map, { Layer, Marker, Source, type LngLatBoundsLike, type MapRef } from 'react-map-gl/mapbox';
...
const normalizeCoordinates = (coordinates: ReadonlyArray<Coordinate>): ParsedCoordinates => {
...
const buildFeatureCollection = (geometry: LineString): FeatureCollection<LineString> => ({
...
const sliceRouteByProgress = (coordinates: LngLat[], progress: number) => {
  ...
  for (let i = 1; i < coordinates.length; i += 1) {
    ...
    if (traveled + segmentLength >= targetLength) {
      ...
      sliced.push([prevLng + (nextLng - prevLng) * ratio, prevLat + (nextLat - prevLat) * ratio]);
      break;
    }
...
const RouteMarker = ({ point, index, renderContent }: RouteMarkerProps) => {
  ...
  useEffect(() => {
    ...
    const controls = animate(element, { opacity: 1, marginTop: '0px' }, { duration: 0.35, ease: 'easeOut' });
  ...
  useEffect(() => {
    ...
    const element = marker.getElement();
    const controls = animate(element, { opacity: 0 }, { duration: 0.2, ease: 'easeIn' });
    controls.finished.then(() => safeToRemove?.());
    return () => controls.stop();
  }, [isPresent, safeToRemove]);

  return (
    <Marker ref={markerRef} longitude={point[0]} latitude={point[1]} anchor="bottom">
...
export function MapRoute({
  ...
  const [animatedProgress, setAnimatedProgress] = useState(() => (progress == null ? 1 : clampProgress(progress)));
  ...
  const visibleRouteData = useMemo(() => {
    ...
    const sliced = sliceRouteByProgress(fetchedRouteCoordinates, clampedProgress);
    if (sliced.length < 2) {
      return null;
    }
  ...
  const renderMarkerContent = (point: LngLat, index: number, isVisible: boolean) => {
    if (MarkerComponent) {
      return <MarkerComponent point={point} index={index} isVisible={isVisible} />;
    }
    return null;
  };
  ...
  useEffect(() => {
    ...
    const fetchRoute = async () => {
      ...
      try {
        const url = new URL(`https://api.mapbox.com/directions/v5/mapbox/driving/${parsedRoute.serialized}`);
        ...
        const message = error instanceof Error ? error.message : 'Failed to load directions.';
  ...
  }, [mapboxToken, parsedRoute.error, parsedRoute.points.length, parsedRoute.serialized]);
  ...
  }, [containerSize.height, containerSize.width, isMapReady, routeMeta]);
  ...
  return (
    <div
      ref={containerRef}
      className={cn('relative h-130 w-full overflow-hidden rounded-lg border border-border', className)}
    >
      <Map
        ref={mapRef}
          ...
          {parsedRoute.points.map((point, index) => {
            ...
            return (
              <RouteMarker
                ...
                point={point}
                index={index}
                renderContent={(targetPoint, targetIndex) => renderMarkerContent(targetPoint, targetIndex, true)}
              />
            );
          })}
        </AnimatePresence>
      ...
      {isLoading || routeError ? (
        StatusBadgeComponent ? (
          <StatusBadgeComponent label={routeError ?? 'Loading route...'} isLoading={isLoading} />
        ) : (
          <div
            className={cn(
